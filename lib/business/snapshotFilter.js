...
const { frequencyPriorities, snapshotValidityTestingMethods }  = require('../constants')
...
const filterOverdueStatusesByPriorities = overdueStatuses => {
  ...
  return pools.map(pool => {
    const frequency = R.compose(
      ...
      R.map(frequency => ({ frequency, priority: frequencyPriorities[frequency.type]})),
      R.map(status => status.frequency),
...
const filterOutYoungerStatusesAlreadyCovered = statuses => {
  return statuses
}
...
const getOverdueStatuses = (date, zmanConfig, snapshots) => {
  ...
  const meaninigfulStatuses = filterOutYoungerStatusesAlreadyCovered(overdueStatuses)

  return filterOverdueStatusesByPriorities(meaninigfulStatuses)
}
