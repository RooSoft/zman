...
const parseSnapshots = ({poolName, frequency, output}) => {
  const cleanup = R.pipe(
    R.split('\n'),
    R.drop(1)
  )

  const snapshots = cleanup(output)
    .filter(line => line.trim().length > 0)
    .map(line => parseSnapshot(line))
    .filter(snapshot => frequency ? snapshot.frequency === frequency : snapshot.frequency !== undefined)
    .filter(snapshot => poolName ? snapshot.poolName === poolName : true)
